...
<div className='w-full h-auto  fixed top-0 left-0 bg-white   z-[110]'>
   <div className='flex items-center justify-between 2xl:py-4 py-6 bg-white w-full md:pl-0 sm:pl-24 pl-16 md:px-0 px-5  md:w-11/12 2xl:w-10/12 mx-auto'>
    ...
    <div onClick={() => setOpen(!Open)} className='text-4xl absolute top-5 left-5 sm:left-10 cursor-pointer md:hidden'>
    <ion-icon name={Open ? 'close' : 'menu'}></ion-icon>
    </div>
    <ul className={`md:flex md:items-center gap-7 absolute md:static bg-white w-full md:z-auto z-[-2] left-0 md:w-auto md:pl-0 pl-10 transition-all duration-500 ease-in ${Open ? 'top-[70px] opacity-100' : 'top-[-300px] md:opacity-100 opacity-100'}`}>
      {Links.map((Link) =>(
        <li className=' md:my-0 my-5 md:ml-3  text-xl'>
     <a href={Link.link} className="hover:scale-110 transition-transform cursor-pointer">{Link.name}</a>
      </li>
      ))}     
    ...
    <input type="search" onChange={handleChange} value={usedata.search} name='search' placeholder='Search for Products' className='w-full h-auto p-3 bg-transparent outline-none ' />
    </div>
    </ul>
